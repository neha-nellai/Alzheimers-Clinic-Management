...
import os
from .forms import TestForm, TestForm1
import pickle

bg_video = os.path.join('static/assets', 'pexels-tima-miroshnichenko-6010766.mp4')
print(bg_video)

def choose_test(request):
    patient=models.Patient.objects.get(user_id=request.user.id)
    return render(request,"hospital/choose_test.html",{'patient':patient})

def upload_data(request):
    l=[]
    inp = []
    res = {0:'Non demented',1:'Demented'}
    ans = ""
    form = TestForm1()
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            l.append(ASF)
        inp.append(l)
        Pkl_Filename = "hospital/data.pkl" 
        with open(Pkl_Filename, 'rb') as file:  
            Pickled_Model = pickle.load(file)
        y_pred = Pickled_Model.predict(inp)
        ans = res[int(y_pred)]
    context = {'form':form,'ans':ans}
    return render(request,"hospital/test1.html",context)

def upload_image(request):
    print("video",bg_video)
    if request.method == 'POST':
        form = TestForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            file = request.FILES['scan']
            imvar = tf.keras.preprocessing.image.load_img(os.path.join("static/uploads", secure_filename(file.name))).resize((176, 176))
            imarr = tf.keras.preprocessing.image.img_to_array(imvar)
            imarr = np.array([imarr])
            model = tf.keras.models.load_model("hospital\model")
            ...
            context = {'type':"Patient is suffering from "+ str(classdict[classcount]),'form':form,'bg_video':bg_video}
            return render(request,'hospital/test.html',context)
    else:
        form = TestForm()
        return render(request, 'hospital/test.html', {'form' : form,'type':"Upload image for test results",'bg_video':bg_video})
